 ...
 * 用法: node scripts/smoke-guards.cjs
 */
const path = require('path');
const { ensureBrowser, newPage } = require('../dist/core/browser.js');
...
(async () => {
  ...
  // 4. 真实导航冒烟（允许失败：网络/登录态受限时不影响注入层结论）
  try {
    ...
    check('真实导航（网络受限可跳过）', true, `goto 异常: ${e.message.slice(0, 80)}`);
  }
