...
| 详情档案探针 | `inspect` | ✅ 实测通过 | **仅搜索池来源**：主动搜索候选人 → 开详情 → 结构化 JSON；`--hi` 执行「立即Hi聊」（耗点数，不是普通文字回复） |
| 详情档案探针 | `talent-detail` | ✅ 实测通过 | **仅投递/聊天/人才管理来源**（非搜索池）提取结构化 JSON；`--hi` 点击免费「回复」打开沟通面板，不发送普通文字 |
